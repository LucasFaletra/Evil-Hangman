...
Status my_string_extraction_test(char* buffer, int length)
{
  ...
  if(my_string_extraction(hString, fp) != SUCCESS || *my_string_at(hString, 1) != 'a')
    {
      fclose(fp);
      my_string_destroy(&hString);
...
Status my_string_push_back_test(char* buffer, int length)
{
  ...
  if(my_string_get_size(hString) != before_push + 1 && my_string_at(hString, my_string_get_size(hString)))
    {
      my_string_destroy(&hString);
...
Status my_string_pop_back_with_empty_string(char* buffer, int length)
{
  MY_STRING hString = NULL;
  hString = my_string_init_default();
  if(my_string_pop_back(hString) != FAILURE || my_string_get_size(hString) < 0)
    {
      my_string_destroy(&hString);
...
Status my_string_c_str_test(char* buffer, int length)
{
  ...
  if(my_string_c_str(hString) == NULL && my_string_c_str(hString)[12] == 'a')
    {
      my_string_destroy(&hString);
...
Status my_string_concat_test(char* buffer, int length)
{
  ...
  if(my_string_get_size(hString1) != 13 || my_string_get_capacity(hString1) <= 13)
    {
...
Status my_string_concat_with_one_empty_string(char* buffer, int length)
{
  ...
  if(my_string_concat(hString1, hString2) != SUCCESS || *my_string_at(hString1, 1) != 'n')
    {
      my_string_destroy(&hString1);
      my_string_destroy(&hString2);
